...
| 12 | Reset on success | Correct login after failures | Key deleted (`nil`) | ⏳ Not yet run |
| 13 | Window expiration | Wait / manual EXPIRE | Counter resets | ⏳ Not yet run |

*(Update the "Status" column with ✅/❌ after running each test.)*

**Note:** the initial test run was affected by an unrelated backend bug (HTTP 500 on `/auth/login` due to a pending Prisma migration — see `RATE_LIMITING.md`). Tests 9-11 were re-run after the fix (`backend_migrate` service added to `docker-compose.yml`) and confirmed correct with the backend fully functional. Tests 12-13 are still pending.
